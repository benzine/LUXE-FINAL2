...
/* ── Selector hooks ────────────────────────────────────────────────
   Components should use these instead of useConfig() when they only need
   a specific slice of state. Prevents mass re-renders on every edit. */
export function useConfigSelector<T>(selector: (cfg: SiteConfig) => T): T {
  return useSyncExternalStore(
    configStore.subscribe,
    () => selector(configStore.get()),
    () => selector(DEFAULT_CONFIG)
  );
}

export function useDesign(): DesignConfig {
  return useConfigSelector((c) => c.design);
}

export function useSlots(): Slot[] {
  return useConfigSelector((c) => c.slots);
}

export function useSalon(): SiteConfig["salon"] {
  return useConfigSelector((c) => c.salon);
}

export function useHeadings(): SiteConfig["headings"] {
  return useConfigSelector((c) => c.headings);
}

export function useStages(): Stage[] {
  return useConfigSelector((c) => c.stages);
}
