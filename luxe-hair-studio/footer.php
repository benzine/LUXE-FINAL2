...
<script>
/* ═══════════════════════════════════════════════════════════════════
   RUNTIME SAFETY GUARD — prevents infinite re-render loops from
   taking down the page. Catches React #185 and breaks cascades.
   ═══════════════════════════════════════════════════════════════════ */
(function () {
  /* ── Fix #1: Patch Object.is for store snapshots ──
     React's useSyncExternalStore uses Object.is to compare snapshots.
     The builder store's useBuilder() returns { ...state, ...methods }
     as a NEW object every call, making Object.is always report "changed"
     → infinite re-renders (React #185). We detect objects that look like
     a store snapshot (have a `subscribe` method) and use shallow equality
     instead of reference equality for those specific comparisons. */
  var originalIs = Object.is;
  Object.is = function (a, b) {
    if (a === b) return true;
    /* Only apply shallow-equal to objects that look like store snapshots
       (have a `subscribe` function) — never affect normal comparisons. */
    if (a && b && typeof a === 'object' && typeof b === 'object' &&
        typeof a.subscribe === 'function' && typeof b.subscribe === 'function') {
      return shallowEqual(a, b);
    }
    return originalIs.call(Object, a, b);
  };
  function shallowEqual(objA, objB) {
    ...
    for (var i = 0; i < keysA.length; i++) {
      var key = keysA[i];
      if (!originalIs.call(Object, objA[key], objB[key])) return false;
    }
    return true;
  }
  ...
  window.addEventListener('error', function (e) {
    var msg = (e.message || String(e.error || '')).toString();
    if (msg.indexOf('#185') !== -1 || msg.indexOf('Maximum update depth') !== -1) {
      reactErrors++;
      var now = Date.now();
      if (now - lastErrorTime < 1000 && reactErrors >= 3) {
        /* Hard loop detected — break it by disabling rapid store updates */
        if (window.__LUXE_STORE_GUARD__) {
          window.__LUXE_STORE_GUARD__.frozen = true;
        }
        /* Show a clear diagnostic instead of a blank page */
        setTimeout(function () {
          var root = document.getElementById('root');
          if (root && root.children.length === 0) {
            root.innerHTML = '<div style="padding:3rem;text-align:center;font-family:Georgia,serif;background:#241c1d;color:#f2e9e1;min-height:100vh;">' +
              '<p style="font-family:ui-monospace,Menlo,monospace;font-size:11px;letter-spacing:.32em;color:#c9b037;margin:0;">RUNTIME SAFETY GUARD</p>' +
              '<h1 style="font-size:2rem;font-weight:500;margin:1rem 0;">Infinite render loop detected and halted.</h1>' +
              '<p style="color:#c0aea4;max-width:520px;margin:0 auto 1.5rem;line-height:1.6;">' +
              'This is caused by the new 3-rail builder features. The Zustand store hook <code style="background:rgba(255,255,255,.08);padding:2px 6px;border-radius:4px;">Ae()</code> ' +
              'returns the entire state object, and components using it in <code style="background:rgba(255,255,255,.08);padding:2px 6px;border-radius:4px;">useEffect</code> dependencies ' +
              'create cascading re-renders. Apply the source fixes below to resolve permanently.</p>' +
              '<a href="javascript:location.reload()" style="display:inline-block;margin-top:1rem;padding:.85rem 1.9rem;border:1px solid #c9b037;border-radius:999px;color:#2b1f1f;background:#c9b037;font-family:ui-monospace,Menlo,monospace;font-size:11px;letter-spacing:.2em;text-transform:uppercase;text-decoration:none;">Reload</a>' +
              '</div>';
          }
        }, 500);
      }
